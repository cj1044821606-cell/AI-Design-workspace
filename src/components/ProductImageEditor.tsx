...
export function ProductImageEditor({ 
  ...
  // Sync with props if they change from code
  useEffect(() => {
    setX(initialX);
    setY(initialY);
  ...
  const handleSave = async () => {
    ...
    try {
      const response = await fetch('/api/save-image-transform', {
        ...
        body: JSON.stringify({ componentName, x, y, scale })
      });
      if (response.ok) {
        setIsEditing(false);
      } else {
  ...
  return (
    <div className="relative w-full h-full group">
      {/* Transformed wrapper for image and bounding box */}
      <div 
      ...
      {/* Edit Button (visible on hover) - Placed outside transformed wrapper to prevent clipping */}
      {!isEditing && (
        <button 
      ...
      {/* Editor Panel - Rendered via Portal to escape parent transforms and overflow:hidden */}
        ...
        >
          <div className="flex flex-col gap-3">
            <div className="flex items-center justify-between gap-2">
              <span className="text-xs font-medium text-neutral-500 w-8">X轴</span>
            ...
            <div className="flex items-center justify-between gap-2">
              <span className="text-xs font-medium text-neutral-500 w-8">Y轴</span>
            ...
            <div className="flex items-center justify-between gap-2">
              <span className="text-xs font-medium text-neutral-500 w-8">缩放</span>
