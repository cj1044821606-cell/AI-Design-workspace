...
import bgImage from '@/画册素材库/背景素材/画册-IHY-15KH3-G1系列-产品页背景.jpg';
import productImage from '@/画册素材库/产品素材/IHY-15KH3-G1&IHY-20KH3-G1&IHY-25KH3-G1&IHY-30KH3-G1.png';

const models = ['IHY-15KH3-G1', 'IHY-20KH3-G1', 'IHY-25KH3-G1', 'IHY-30KH3-G1'];
...
const rows: TableRow[] = [
  ...
  { kind: 'row', label: 'Max. Efficiency', values: ['97.80%'] },
  { kind: 'row', label: 'EU Efficiency', values: ['97.20%'] },
  { kind: 'row', label: 'Battery Discharge Max. Efficiency', values: ['98.50%'] },
  { kind: 'row', label: 'PV Charge Max. Efficiency', values: ['97.50%'] },

  { kind: 'section', label: 'PV Side' },
  { kind: 'row', label: 'Max. PV Array Power (kWp)', values: ['30', '40', '50', '50'] },
  { kind: 'row', label: 'Max. PV Input Voltage (V)*', values: ['1000*'] },
  { kind: 'row', label: 'Rated PV Input Voltage (V)', values: ['620'] },
  { kind: 'row', label: 'Start-up Voltage (V)', values: ['135'] },
  { kind: 'row', label: 'MPPT Operating Voltage Range (V)*', values: ['200-950*'] },
  { kind: 'row', label: 'Number of MPPT Trackers', values: ['4'] },
  { kind: 'row', label: 'Number of Strings per MPPT', values: ['1'] },
  { kind: 'row', label: 'Max. Input Current per MPPT (A)', values: ['20'] },
  { kind: 'row', label: 'Max. Short Circuit Current per MPPT (A)', values: ['25'] },

  { kind: 'section', label: 'Battery Side' },
  { kind: 'row', label: 'Battery Type', values: ['Lithium-ion'] },
  { kind: 'row', label: 'Battery Voltage Range (V)', values: ['150-840'] },
  ...
  { kind: 'row', label: 'Rated Output Current (A)', values: ['22.7/21.7/20.8', '30.2/28.8/27.6', '37.7/36.1/34.6', '45.6/43.3/41.7'], compact: true },
  { kind: 'row', label: 'On/Off-grid Switch Time (ms)', values: ['< 10'] },
  ...
  { kind: 'section', label: 'Protection' },
  {
    ...
    values: [
      'DC reverse polarity protection; Battery input reverse connection protection; Insulation resistance protection; Surge protection (DC/AC: Type II/Type II); Over temperature protection; Residual current protection; Islanding protection; AC overvoltage protection; Overload protection; AC Short Circuit protection',
    ],
    compact: true,
  },
  ...
  { kind: 'row', label: 'Noise Level (dB)', values: ['< 45'] },
...
function renderTableRow(row: TableRow) {
  if (row.kind === 'section') {
    return (
      <tr key={row.label} className="bg-[#cac9c8] text-[#231A16] font-bold text-left">
        <td colSpan={models.length + 1} className="border-[0.368pt] border-white h-[4.2mm] px-[1.2mm] align-middle">
          {row.label}
        </td>
      </tr>
    );
  ...
  const cellClass = row.compact
    ? 'border-[0.368pt] border-white py-[1.0mm] px-[1.2mm] align-middle leading-[1.12]'
    : 'border-[0.368pt] border-white h-[4.2mm] px-[1.2mm] align-middle';
...
export default function CatalogSpreadGlobal20260420_30B() {
  return (
    <div className="w-[420mm] h-[297mm] bg-white relative overflow-hidden shadow-sm flex">
      <div className="w-[210mm] h-full relative z-10">
        ...
        <div className="design-group-wrapper absolute left-[86.7mm] top-[96.84mm] w-[108.83mm] h-[103.22mm] z-10">
          <div className="design-group-label">Product Image</div>
          <ProductImageEditor
            src={productImage}
            componentName="CatalogSpreadGlobal20260420_30B"
            initialX={34} /* IMAGE_TRANSFORM_X */
            initialY={-68} /* IMAGE_TRANSFORM_Y */
            initialScale={0.72} /* IMAGE_TRANSFORM_SCALE */
            defaultHeight="100%"
            defaultMarginLeft="0"
          />
        </div>
      ...
      <div className="w-[210mm] h-full relative">
        <div className="design-group-wrapper absolute left-[38.151mm] top-[15.63mm] w-[150.099mm] z-10">
          ...
          <table className="w-full border-collapse text-[4.6pt] text-center" style={{ fontFamily: '"Mark OT", sans-serif' }}>
            <colgroup>
              <col style={{ width: '38%' }} />
            ...
            <tbody>
              <tr className="bg-[#9e9e9e] text-white font-bold">
                <td className="border-[0.368pt] border-white h-[4.551mm] px-[0.9mm] align-middle whitespace-nowrap">Model</td>
                {models.map((model) => (
                  <td key={model} className="border-[0.368pt] border-white h-[4.551mm] px-[0.9mm] align-middle whitespace-nowrap text-[4.2pt]">
                    {model}
                  </td>
                ))}
              </tr>
